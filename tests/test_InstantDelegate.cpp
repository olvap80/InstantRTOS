...
TEST_CASE("InstantDelegate simple"){
    ...
    CHECK( 4171 == CustomAPI(MyCallback::Unstorable(SomeFunctor{2}), 4000) );
}
...
namespace{
    ...
    int function_to_bind_receivingRef(const TestClass& t, int val){
        return t.valueGet() + 100 + val;
    }
    int function_to_bind_receivingPtr(TestClass* t, int val){
        return t->valueGet() + 1000 + val;
    }
    int function_to_bind_receivingConstPtr(const TestClass* t, int val){
        return t->valueGet() + 10000 + val;
    }
} //namespace
...
TEST_CASE("InstantDelegate advanced"){
    ...
    // Ensure function taking const pointer works via const object
    const TestClass demoCallable9{9};
    CHECK(
        30019 == test(
                    Delegate<int(int)>::From(&demoCallable9)
                        .Bind<&function_to_bind_receivingConstPtr>()
                )
    );
}

TEST_CASE("InstantDelegate null and equality semantics"){
    using D = Delegate<int(int)>;

    // Construct null delegate via simple-case ctor with nullptr
    D nullDel(static_cast<D::SimpleCaseCallee*>(nullptr));
    CHECK( !nullDel );

    // Function and method delegates compare unequal when targets differ
    int (*fptr)(int) = function_to_pass_to_Delegate;
    D fromFunc(fptr);

    struct Obj{ int test_method(int x) const { return x; } } o;
    D fromMethod = D::From(&o).Bind<&Obj::test_method>();

    CHECK( fromFunc != fromMethod );

    // one is less, the other is greater (we know they differ but not how)
    bool lt1 = fromFunc < fromMethod;
    bool lt2 = fromMethod < fromFunc;

    CHECK( lt1 + lt2 == 1 );  // exactly one is true
}
