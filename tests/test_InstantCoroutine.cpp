...
TEST_CASE("InstantCoroutine panic when reaching CoroutineEnd without Stop"){
    // Define a coroutine that yields once then reaches CoroutineEnd
    CoroutineDefine( OneThenEnd ){
        CoroutineBegin(int)
            CoroutineYield(42);
            // intentionally fall through to CoroutineEnd without CoroutineStop
        CoroutineEnd()
    };

    OneThenEnd co;
    // First call yields a value
    CHECK( co() == 42 );
    // Next call must panic because coroutine reaches CoroutineEnd
    CHECK_THROWS_AS(
        [&]{ co(); }(),
        TestInstantCoroutinePanicException
    );
