...
#if 0
/* TODO: that stuff is broken so far, use SimpleDebounce
   so disable those tests for now */

TEST_CASE("InstantDebounce: DebounceAction with Scheduler"){
    Scheduler sch;
    sch.Start(0);

    bool raw = false;
    auto checker = Delegate<bool()>([&](){ return raw; });

    // require 3 consecutive checks at 10 ticks interval
    DebounceAction deb(false, 10, 3);

    bool sawTrue = false;
    bool sawFalse = false;
    deb.OnTrue(Delegate<void()>([&](){ sawTrue = true; }))
       .OnFalse(Delegate<void()>([&](){ sawFalse = true; }));

    deb.Schedule(sch, checker);

    // Initially nothing happens until schedule time comes
    CHECK( sch.ExecuteAll(0) == false );

    SUBCASE("No chatter transitions to true then back to false"){
        raw = true;
        // run for 2 intervals: not enough
        CHECK( sch.ExecuteAll(10) );
        CHECK( sch.ExecuteAll(20) );
        CHECK( !sawTrue );

        // third interval: should flip to true and call OnTrue
        CHECK( sch.ExecuteAll(30) );
        CHECK( sawTrue );
        CHECK( deb.Value() == true );

        // Now flip raw back to false and accumulate 3 checks
        raw = false; sawTrue = false; sawFalse = false;
        CHECK( sch.ExecuteAll(40) );
        CHECK( sch.ExecuteAll(50) );
        CHECK( !sawFalse );
        CHECK( sch.ExecuteAll(60) );
        CHECK( sawFalse );
        CHECK( deb.Value() == false );
    }

    SUBCASE("Chatter near boundary prevents transition"){
        raw = true;
        CHECK( sch.ExecuteAll(10) );
        CHECK( sch.ExecuteAll(20) );
        // chatter resets count
        raw = false;
        CHECK( sch.ExecuteAll(30) );
        raw = true;
        CHECK( sch.ExecuteAll(40) );
        CHECK( !sawTrue );
        // need two more consecutive after reset
        CHECK( sch.ExecuteAll(50) );
        CHECK( sch.ExecuteAll(60) );
        CHECK( sawTrue );
        CHECK( deb.Value() == true );
    ...
    SUBCASE("Cancel prevents further callbacks"){
        raw = true;
        CHECK( sch.ExecuteAll(10) );
        CHECK( sch.ExecuteAll(20) );
        deb.Cancel();
        // even though time advances, callbacks should not fire
        CHECK( sch.ExecuteAll(30) == false );
        CHECK( !sawTrue );
        CHECK( deb.Value() == false );
    }
}
